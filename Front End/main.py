...
def main(page: ft.Page):
    ...
    def btn_return(e):
        ...
        # page.session.set("user", user)
        group_id = page.session.set("group_id", "0")
        page.route = "/chatlist"
        page.update()
    ...
    def route_change(route):
        ...
        if page.route == "/chatlist": 
                ...
                group = ft.Row(
                    [
                        ft.Container(
                            ...
                            width=200,
                            content=ft.Text(value=i['group'], color='#181D27'),
                        ),
                        ft.Container(
            ...
            db = UsersDB()
            user_list = []
            for i in db.read_user():
                user = ft.Row(
                    [   
                        ft.Container(
                            alignment=ft.alignment.center_left,
                            width=200,
                            content=ft.Text(value=i['user'], color='#181D27'),
                        ),
                        ft.Container(
                            alignment=ft.alignment.center_right,
                            width=440,
                            content=ft.ElevatedButton(
                                    ...
                                    bgcolor="#FFB347",
                                    on_click=join_chat,
                                    # url=page.route.replace('list', '') + '/' + str(i['user']),
                                    # url_target='_self'
                                ),
                        ),
                    ],
            ...
            page.add(
                ft.Column(
                        [
                            ft.Text(value="Group Chat List", color=ft.colors.WHITE),
                            ft.Container(
                                content=ft.Column(group_list),
            ...
            page.add(
                ft.Column(
                        [
                            ft.Text(value="Private Chat List", color=ft.colors.WHITE),
                            ft.Container(
                                content=ft.Column(user_list),
                                bgcolor=ft.colors.with_opacity(1, '#F1F1F1'),
                                border_radius=10,
                                width=700,
                                padding=20,
        ...
        # if troute.match('/chat/:user'):
        #     page.session.set('user', troute.user)
        #     print(troute.user)
        #     if page.session.contains_key("user"):
        #         page.clean()
        #         # page.session.set("group_id", troute.group_id)s
